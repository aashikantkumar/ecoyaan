...
const liquidbuttonVariants = cva(
  "relative inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 whitespace-nowrap font-semibold outline-none transition-all duration-200 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0 focus-visible:ring-2 focus-visible:ring-slate-300",
  {
    variants: {
      variant: {
        primary:
          "bg-slate-900 text-white shadow-lg shadow-slate-900/20 hover:bg-slate-800 active:translate-y-px active:bg-slate-950",
        secondary:
          "border border-slate-300 bg-white text-slate-700 hover:bg-slate-100",
        ghost: "bg-transparent text-slate-700 hover:bg-slate-100",
        link: "h-auto rounded-none bg-transparent px-0 text-slate-700 underline-offset-4 hover:underline",
        destructive:
          "bg-red-600 text-white hover:bg-red-700 active:bg-red-800",
      },
      size: {
        default: "h-10 rounded-full px-4 py-2 text-sm",
        sm: "h-8 rounded-full px-3 text-xs",
        lg: "h-11 rounded-full px-6 text-sm",
        xl: "h-12 rounded-full px-8 text-base",
        xxl: "h-14 rounded-full px-10 text-base",
...
function LiquidButton({
  ...
  const Comp = asChild ? Slot : "button"
  const useGlassEffect = variant === "primary"
